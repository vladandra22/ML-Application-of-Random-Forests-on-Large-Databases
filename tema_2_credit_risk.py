...
def preprocesare_data(df, categorical_columns, numeric_columns):
    ...
    atr_redundante_categoric = [b for (p, a, b) in redundante_categorical[:5] if b != 'loan_approval_status' and b in df.columns]
...
def train_si_eval_model_random_forest(train_data, test_data, full_data, numeric_columns, categorical_columns):
    ...
    X_train = train_data.drop(columns=['loan_approval_status'])
    Y_train = train_data['loan_approval_status']

    X_test = test_data.drop(columns=['loan_approval_status'])
    Y_test = test_data['loan_approval_status']
...
def solve():
    full_filepath = 'credit_risk_full.csv'
    train_filepath = 'credit_risk_train.csv'
    test_filepath = 'credit_risk_test.csv'
    ...
    plot_frecventa_clase(df_train, 'loan_approval_status', 'antrenare')
    plot_frecventa_clase(df_test, 'loan_approval_status', 'testare')
